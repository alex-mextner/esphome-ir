...
void HaierClimate::setup_ir_cmd() {
  if (this->mode == climate::CLIMATE_MODE_OFF) {
    // Кастомный код выключения для HSU-12HT303/R2
    uint8_t off_state[kHaierACYRW02StateLength] = {
      0xA6, 0x90, 0x00, 0x02, 0x00, 0xA0, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x05
    };
    ac_->setRaw(off_state);
  } else {
    ac_->on();
